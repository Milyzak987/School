    ...
    while (q--) {
        ...
        if (type == 1) {
...

            {
                ...
                auto it = positions[v].find(p);

                int prev_idx = 0;
                if (it != positions[v].begin()) prev_idx = *prev(it);

                updatePrev(p, prev_idx);

                auto next_it = next(it);
                if (next_it != positions[v].end()) {
                    updatePrev(*next_it, p);
                }
            }
